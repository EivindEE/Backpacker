...
function search(){
	...
	if(searchComplete($result)){
		$toReturn = buildReturn($result,'viewLocation.php','id');
	}		
	...
	if(!searchComplete($result)){
		...
		if(searchComplete($result)){
			$toReturn = buildReturn($result,'region.php','id');
		}		
	}
	...
	if(!searchComplete($result)){
		...
		if(searchComplete($result)){
			$toReturn = buildReturn($result,'country.php','c_name');
		}
	}
	...
	if(!searchComplete($result)){
		...
		if(searchComplete($result)){
			$toReturn = buildReturn($result,'user.php','uname');
		}
	}
...
function buildReturn($query,$id,$field){
	...
	$toReturn = $id ."?id=". $regionToReturn[$field];
	return $toReturn;
}
